...
async function main() {
    ...
    // Count total test cases (not just files)
    let totalTestCases = 0;
    for (const [_, markdown] of finalEntries.entries()) {
        // Count all test case headings (### ✓, ### ⚠️, ### ❌)
        const testCaseMatches = markdown.match(/^###\s+[✓⚠️❌]/gm) || [];
        totalTestCases += testCaseMatches.length;
    }
    
    console.log(`📊 DOCUMENTATION STATS:`);
    console.log(`   Test Files:        ${finalEntries.size}`);
...
`;
    ...
    console.log(`TOTAL_TEST_CASES:${totalTestCases}`);
